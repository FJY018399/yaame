 ...
 * @FilePath: /yaame-h5/src/pages/myIncome/myIncome.vue
...
<template>
	<div :style="{ paddingTop: safe_height + 1.08 + 'rem' }">
        ...
        <p>The data is refreshed every 10 minutes or so</p>
        <p>The time here is the time in UTC 0 time zone</p>
        <p v-if="isTodayFlag">Remaining until the end of today: {{ timeText }}</p>

		<div v-if="active === 'DailyIncome'" class="info-content">
			<div>
				<span class="label">Revenue Diamonds:</span>
				<span class="value">{{ dayInfo.diamonds_income }}</span>
			</div>
			<div>
				<span class="label">Length of live broadcast:</span>
		...
		<div v-else class="info-content">
			...
			<div>
				<span class="label">Live broadcast days: </span>
				<span class="value">{{ mouthInfo.mike_day + "days" }}</span>
			</div>
		</div>

		<p class="list-info">Detailed information</p>
