    ...
    export default {
        ...
        methods: {
            ...
            async initData() {
                ...
                const res = await rechargeOrderCheck(options);
                if (res.status === 1000) {

                    this.pageLoading = false;
                    let status = res.data.status;
                    if (['PAYED','CANCELED','FAIL', 'SUCCESS'].indexOf(status) === -1) {
                        status = 'FAIL';
                    }

                    this.status = status;
                    if (isIOS()) {
                        window.webkit.messageHandlers.onWebPayResult.postMessage(status);
                    } else {
                        window.peanut_client.onWebPayResult(status);
                    }
                    // window.peanut_client.onWebPayResult(status);
                } else if (res.status === 6307) {
                    this.pageLoading = false;
                    this.status = 'FAIL';
                }
            },
