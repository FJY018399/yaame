<template>
	<div :style="{ paddingTop: safe_height + 1.08 + 'rem' }" class="main">
		...
		<popup v-if="quitVisible" class="flex-box">
          ...
          Resigning from the guild will prevent you from joining the new guild for a month. <br />
          Confirm to exit the guild?
				</div>
				<div class="container-footer">
					<div @click="handelQuit">Confirm</div>
					<div @click="quitVisible = false;">Cancel</div>
				</div>
			</div>
		</popup>
		...
		<popup v-if="deleteVisible" class="flex-box">
			<div class="container">
				<p class="container-title">Tips</p>
				<div class="container-content">
					Are you sure to remove "{{ selectUser.nick_name }}" from your agency?
					<br />
					His salary can not be settled if he was removed from the agency
				</div>
				<div class="container-footer">
					<div @click="deleteConfirm">Confirm</div>
					<div @click="deleteVisible = false;">Cancel</div>
				</div>
			</div>
		</popup>
	</div>
	...
	methods: {
		...
		submit() {
			if (this.is_guild_owner === 0) {
				...
				location.href = "./GuildInvite.html";
			}
		},
		async handelQuit() {
			try {
