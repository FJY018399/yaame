<template>
	...
	>
		...
		<p class="info-p" style="margin-bottom: 0.4rem">
			{{ $t("dataRefreshTip2", [timeText]) }}
		</p> -->

        <p class="info-p">{{ $t('dataTip') }}</p>
        <p class="info-p">{{ $t('dataRefreshTip2', [timeText]) }}</p>

		<div v-if="active === 'DailyIncome'">
			<div class="info-container">
				<span>{{ $t('getUSD') }}</span>
				<span>{{ dayInfo.total_income }}</span>
			</div>
			<div class="info-container">
				<span>{{ $t('validNumber6') }}</span>
				<span>{{ dayInfo.today_effect_num }}</span>
			</div>
			<div class="info-container">
				<span>{{ $t('validNumber3') }}</span>
				<span>{{ dayInfo.second_effect_num }}</span>
			</div>
		</div>
		...
		<div v-else>
			<div class="info-container">
				<span>{{ $t('getUSD') }}</span>
				<span>{{ mouthInfo.total_income }}</span>
			</div>
			<div class="info-container">
				<span>{{ $t('validNumber6') }}</span>
				<span>{{ mouthInfo.today_effect_num }}</span>
			</div>
			<div class="info-container">
				<span>{{ $t('validNumber3') }}</span>
				<span>{{ mouthInfo.second_effect_num }}</span>
			</div>
		</div>
		<!-- <div class="search">
			<input
		...
		<!-- 每日数据 -->
		<div v-if="active === 'DailyIncome' && listShow" class="month" ref="topDay">
			<van-list
				v-if="!showEight"
				ref="contentHeight"
				v-model="loading"
				...
				<table class="table-header table-header-month">
					<thead>
						<tr>
							<th></th>
							<th>{{ $t('matchNumber') }}</th>
							<th>{{ $t('roundNumber6') }}</th>
							<th>{{ $t('commissionUSD') }}</th>
							<th>{{ $t('roundNumber3') }}</th>
							<th>{{ $t('commissionUSD') }}</th>
							<th>{{ $t('totalCommissionUSD') }}</th>
							<th>{{ $t('guildCommissionUSD') }}</th>
						</tr>
					</thead>
					<tbody>
						<tr v-for="(item, index) in dayList" :key="index">
							<td style="min-width: 2.3rem">
								<div class="user-info" @click="toDetails(item)">
									<img :src="item.avatar" alt="" />
									<span class="no-wrap">{{ item.nick_name }}</span>
								</div>
							</td>
							<td>{{ item.total_match_num || 0 }}</td>
							<td>{{ item.today_effect_num || 0 }}</td>
							<td>{{ item.today_income || 0 }}</td>
							<td>{{ item.second_effect_num || 0 }}</td>
							<td>{{ item.second_income || 0 }}</td>
							<td>{{ item.total_income || 0 }}</td>
							<td>{{ item.guild_income || 0 }}</td>
						</tr>
					</tbody>
				</table>
			</van-list>
...
import { getGuildIncomeDaily, getGuildIncomeMonth } from "../api";
export default {
	components: {
		Header,
	},
	setup() {
		...
		const getDay = async () => {
			try {
				...
				const res = await getGuildIncomeDaily(options);
				...
				dayInfo.value.total_income = res.data.total_income || 0;
				dayInfo.value.today_effect_num = res.data.today_effect_num || 0;
				dayInfo.value.second_effect_num = res.data.second_effect_num || 0;
		...
		const getMonth = async () => {
			try {
				...
				const res = await getGuildIncomeMonth(options);
				if (res.status === 1002) {
					Toast.fail(res.desc);
					return;
				}
				if (res.status !== 1000) return;
				mouthInfo.value.total_income = res.data.total_income || 0;
				mouthInfo.value.today_effect_num = res.data.today_effect_num || 0;
				mouthInfo.value.second_effect_num = res.data.second_effect_num || 0;
