    ...
    <div class="main">
        ...
        <div class="container" :class="{ bingo: bingoActive }">
            ...
            <div class="content">
                ...
                <img v-show="audioPlaying" class="bgm" :class="{ stop: !audioPlaying }" :src="imgs.bgm" @click="playBGM" />
                <img v-show="!audioPlaying" class="bgm" :class="{ stop: !audioPlaying }" :src="imgs['bgm-stop']" @click="playBGM" />
...
export default {
    ...
    data() {
        return {
            ...
            light: true, // 灯带闪烁控制
            diamondIndex: 1,
            diamondArr: [10, 50, 100, 1000, 5000],
            todayWinner: {}, // 今日冠军
    ...
    created() {
        this.page_loading = !this.isIOS; // IOS不loading
    ...
    mounted() {
        ...
        window.handleH5Event = (data) => {
            if (data) {
                console.log("收到聊天室消息===> 客户端", data);
            }
            this.onChatroomMsgs(JSON.parse(JSON.stringify(data)));
        };
    },
    methods: {
        playBGM() {
        ...
        onChatroomMsgs(msg) {
            let { data } = msg;
            let { match_record_id, command_type, command_args } = data;
            console.log("收到聊天室消息", command_type, JSON.parse(command_args), moment().format("HH:mm:ss"));
            switch (command_type) {
                case "MATCH_START":
                    // 投注阶段开始
                    if (this.waiting) {
